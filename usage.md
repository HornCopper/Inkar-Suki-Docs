...
> 由于企鹅对个人账号的围追堵截，目前暂时停用授权系统的运作，授权命令可用，但不会再检测到期时间。
...
!> 近期受推栏影响，无法查询他人属性，请选择下方的方案进行导入后才可查询。

#### 通过 JCL

在`茗伊插件集-团队-团队工具-战斗事件记录`中，启用，随后勾选`在其他地图中启用`，最小记录周期改为`10秒`，前往木桩使用平A技能攻击10秒，然后在脱战后，点击`打开数据目录`会给出一个文本编辑器，其中有一个路径，将路径复制到本地文件资源管理器，将JCL直接上传（不可改名）至有音卡的群的群文件。

#### 通过 角色统计

在`茗伊插件集-角色统计-装备统计`中，选中自己要导入的角色并切换到正确的装备栏，右上角有`导出`按钮，打开一个文本编辑器。

- 命令：`提交属性 <服务器> <ID> <心法>`

在有音卡的群里发送上述命令，随后在音卡询问后，将游戏内文本编辑器中的内容**原样复制**到聊天框中直接发送。
...
该部分计算器全部由音卡自行研发，更详细的使用方法请参考：[计算器使用指南](https://inkar-suki.codethink.cn/Inkar-Suki-Docs/#/calculator)。

> 注意，双DPS心法或DPS+T双心法可能需要指定计算器，`计算器`命令仅支持单输出心法或输出治疗双心法，`T计算器`仅支持防御心法。`紫霞功`、`太虚剑意`、`天罗诡道`、`惊羽诀`、`幽罗引`请使用`QC`、`JC`、`TL`、`JY`、`WX`作为前缀，例如`QC计算器`。
...
- 将`JCL`文件拖到其他目录，在文件名前方加入`CQC-`（标记这个文件需要音卡分析**且分析器为池清川P2分析器**）。
- 将重命名后的文件上传至群文件，并等待片刻，音卡将会发送`DPS`和`HPS`统计图。

> 池清川分析器的时间范围是BOSS的`龙怒`开始读条，结束时间为池清川脱离战斗，这个时间节点是**绝对准确的**，任何手动清除都没有此方法准确，且在BOSS重伤到脱离战斗这段时间内产生的任何DPS/HPS（均为无意义的数值）都不会进入统计界面。
...
- 从茗伊插件集（团队-团队工具-小齿轮）勾选`战斗事件记录`，勾选`在秘境地图中启用`。
- 在BOSS离开战斗后，点击`打开数据目录`，复制路径，在本地资源管理器中打开。
- 将`JCL`文件拖到其他目录，在文件名前方加入`YXC-`（标记这个文件需要音卡分析**且分析器为尹雪尘承伤治疗统计分析器**）。
- 将重命名后的文件上传至群文件，并等待片刻，音卡将会发送尹雪尘的逐波灵游承伤技能期间的治疗统计。

> 该分析器对每个玩家单独计数，例如灵素心法同时给出25个龙葵自苦，其中只有15个打在了血未满的玩家上，那么计数就只有15。对于已经受到足够治疗的玩家，溢出的治疗不会计数，所以本功能显示的次数会少于魔盒`JCL`分析给出的次数。

### 开怪检查

*First Attacking Lookup*
...
- 从茗伊插件集（团队-团队工具-小齿轮）勾选`战斗事件记录`，勾选`在秘境地图中启用`。
- 在BOSS离开战斗后，点击`打开数据目录`，复制路径，在本地资源管理器中打开。
- 将`JCL`文件拖到其他目录，在文件名前方加入`FAL-`（标记这个文件需要音卡分析**且分析器为开怪记录分析器**）。
- 将重命名后的文件上传至群文件，并等待片刻，音卡将会发送每个`NPC`所受到的前3次技能伤害。

> 请自行斟酌这个技能是否开怪。注意，开怪的技能不一定有数值，0数值技能也可能开怪（例如没有下秘籍的`吞日月`等），请自行判断好，不要直接以结果断定别人开怪。
...
- 从茗伊插件集（团队-团队工具-小齿轮）勾选`战斗事件记录`，勾选`在秘境地图中启用`。
- 在BOSS离开战斗后，点击`打开数据目录`，复制路径，在本地资源管理器中打开。
- 将`JCL`文件拖到其他目录，在文件名前方加入`ROD-`（标记这个文件需要音卡分析**且分析器为重伤记录分析器**）。
- 将重命名后的文件上传至群文件，并等待片刻，音卡将会发送每个重伤玩家最后`5`次受到的有效伤害。

> 未出现技能详情的可能是该玩家在战斗开始前就已重伤，或该玩家重伤起身后到再次重伤期间未收到伤害（杯水留影到期属于此类）。
...
- 从茗伊插件集（团队-团队工具-小齿轮）勾选`战斗事件记录`，勾选`在秘境地图中启用`。
- 在BOSS离开战斗后，点击`打开数据目录`，复制路径，在本地资源管理器中打开。
- 将`JCL`文件拖到其他目录，在文件名前方加入`IKS-`（标记这个文件需要音卡分析**且分析器为剑三警长**）。
