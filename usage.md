...
**音卡是完全脱机的项目，如果出现查询玩家属性音卡告知无法获取的，您可以按照以下方法操作：**

- 点击 茗伊插件集-角色统计-装备统计；
- 将该角色的装备统计勾选上，随后点击到装备统计界面中，选中所需要提交属性的角色；
- 点击右上角的导出按钮；
- 切换到群聊中，发送：`提交属性 服务器 角色名 心法`，音卡会等待给出装备数据；
- 将游戏内点击`导出`按钮后出现的文本框内的内容**原样**复制粘贴发送给音卡。
...
详细的使用方法请参考：[计算器使用指南](https://inkar-suki.codethink.cn/Inkar-Suki-Docs/#/calculator)。

### JCL 分析器

详细的使用方法请参考：[JCL 分析器使用指南](https://inkar-suki.codethink.cn/Inkar-Suki-Docs/#/jcl)。
